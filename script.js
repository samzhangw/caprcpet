...
function showInstructions() {
  ...
  // 顯示側邊關閉按鈕
  setTimeout(() => {
    const mobileFab = document.querySelector('.mobile-close-fab');
    if (mobileFab) {
  ...
  // 重置為第一步
  const firstStep = document.querySelector(".instruction-step[data-step='1']");
  if (firstStep) {
    firstStep.click();
  }
  
  // 應用移動端優化
  enhanceMobileInstructions();
}
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  function switchToStep(stepNumber) {
    ...
    // 更新步驟狀態
    instructionSteps.forEach(step => {
      const step_num = parseInt(step.getAttribute("data-step"));
    ...
    instructionContents.forEach(content => {
      const content_step = parseInt(content.getAttribute("data-step"));
      content.classList.remove("active");
      
      if (content_step === stepNumber) {
        content.classList.add("active");
        content.scrollTop = 0; // 滾動到頂部
      }
    });
    
    // 更新步驟指示器
    stepIndicators.forEach(indicator => {
      const indicator_step = parseInt(indicator.getAttribute("data-step"));
      indicator.classList.remove("active");
      
      if (indicator_step === stepNumber) {
    ...
    nextButton.disabled = (stepNumber === 5);
  }
...
// 增強手機版使用說明的使用體驗
function enhanceMobileInstructions() {
  // 檢測是否為移動設備
  const isMobile = window.innerWidth <= 768;
  
  if (isMobile) {
    const instructionModal = document.getElementById('instructionsModal');
    const contentWrapper = document.querySelector('.instruction-content-wrapper');
    const mobileFab = document.querySelector('.mobile-close-fab');
    
    if (instructionModal && contentWrapper) {
      // 改善滾動體驗
      let touchStartY = 0;
      let lastScrollTop = 0;
      ...
      contentWrapper.addEventListener('touchmove', function(e) {
        ...
        // 如果在頂部並且向下滑動，或者在底部並且向上滑動，阻止默認行為以防止頁面滾動
        ...
        if (question && answer) {
          ...
          question.addEventListener('click', function() {
            ...
            faqItems.forEach(otherItem => {
              ...
              if (otherQuestion && otherQuestion !== this && otherQuestion.classList.contains('active')) {
      ...
      function adjustContentHeight() {
        const modalContent = document.querySelector('.instructions-modal-content');
        if (modalContent) {
          const windowHeight = window.innerHeight;
          const navHeight = document.querySelector('.instructions-nav').offsetHeight || 0;
          const navButtonsHeight = document.querySelector('.instructions-nav-buttons').offsetHeight || 0;
          const confirmButtonHeight = document.querySelector('.confirm-button').offsetHeight || 0;
          const headerHeight = modalContent.querySelector('h2').offsetHeight || 0;
          
          // 設置內容區域的最大高度
          const maxContentHeight = windowHeight * 0.7 - (navHeight + navButtonsHeight + confirmButtonHeight + headerHeight + 40);
          contentWrapper.style.maxHeight = `${maxContentHeight}px`;
        }
      }
